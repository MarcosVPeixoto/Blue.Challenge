...
namespace Blue.Challenge.Business.Handlers.Queries
{
    public class GetContactsQueryHandler(IIdentityService identityService, 
                                        ...
                                        IMapper mapper) 
                                        : IRequestHandler<GetContactsQuery, List<GetContactQueryResponse>>
    {
        private readonly IIdentityService _identityService = identityService;
        private readonly IContactRepository _contactRepository = contactRepository;
        private readonly IMapper _mapper = mapper;

        public async Task<List<GetContactQueryResponse>> Handle(GetContactsQuery request, CancellationToken cancellationToken)
        {
            var userId = _identityService.GetUserId();
            var contacts = await _contactRepository.GetByUserIdAsync(userId);
            return _mapper.Map<List<GetContactQueryResponse>>(contacts);
        }
    }
}
